...
public class CentroCustoResource {
    ...
    public ResponseEntity<List<CentroCustoDTO>> findByUsuario(@PathVariable Long usuarioId) {
        return ResponseEntity.ok().body(centroCustoService.findByUsuario(usuarioId));
    }
    ...
    public ResponseEntity<Void> insert(@Valid @RequestBody CentroCustoDTO objDto) {
        CentroCusto centroCusto = centroCustoService.create(objDto);
