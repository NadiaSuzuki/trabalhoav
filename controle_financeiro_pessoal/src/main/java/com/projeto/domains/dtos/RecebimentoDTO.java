...
public class RecebimentoDTO {
    ...
    private Long id;
    @NotBlank(message = "Data do recebimento necessária")
    @JsonFormat(pattern = "dd/MM/yyyy")
    @Column(nullable = false)
    private LocalDate dataRecebimento = LocalDate.now();
    @Digits(integer = 12, fraction = 3, message = "Valor recebido pode ter até  12 inteiros e 3 decimais")
    @PositiveOrZero(message = "Valor recebido deve ser maior que zero")
    private BigDecimal valorRecebido;
    @NotBlank(message = "Observação é obrigatório")
    @Size(max = 80, message = "A observação pode ter ate 80 caracteres")
